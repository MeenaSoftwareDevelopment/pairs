...
export const useGameLogic = (initialCards) => {
    ...
    const handleCardClick = (cardId) => {
        ...
        const flippedCards = updatedCards.filter(card => card.isFlipped && !card.isMatched && card.id !== cardId);
        ...
        if (flippedCards.length === 1) {
            ...
            if (flippedCards[0].color === clickedCard.color) {
                ...
                updatedCards.find(card => card.id === flippedCards[0].id).isMatched = true;
            } else {
                // if the colours are not matching, after a delay this will flip them back down.
                setTimeout(() => {
                    setCards(prevCards => 
                        prevCards.map(card =>
                            (!card.isMatched && (card.id === cardId || card.id === flippedCards[0].id))
    ...
    return { cards, points, handleCardClick };
};
